...
- **Highlight in place; never dim, hide, or move.** A starred act gets a filled **heart** glyph plus a visually louder block (brighter fill or outline), legible in sunlight and distinguishable from stage-colour differences. Unstarred acts carry the same heart outlined — the standing hint that a block is tappable — but are never dimmed. There is **no favourites-only filter** — the grid always shows the full programme; starred acts merely pop ([0007](./0007-concert-poster-visual-idiom.md)). _Narrowed by [0021](./0021-focus-mode.md): a transient, opt-in Focus does dim unstarred acts._
- **The act block is not a keyboard control.** It stays a plain `div` — pointer cursor, no `tabindex`, no `role`. Making all ~85 acts tabbable would put that many stops between the Header and anything else, which is a worse outcome than the honest limitation. The Header buttons remain proper keyboard-operable controls.
